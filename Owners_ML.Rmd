...
# Perform stratified sampling
 split_data <- stratified(ML_data_no_NA, # Set dataset
                         group = "owned_users", # Set variables to use for stratification
...
dtrain <- xgb.DMatrix(data = as.matrix(train_data[, c(2:6, 8:199)]), 
                      label = train_data$owned_users)
# Create test matrix
dtest <- xgb.DMatrix(data = as.matrix(test_data[, c(2:6, 8:199)]), 
                     label = test_data$owned_users)
```
...
bst <- xgb.cv(data = dtrain, # Set training data
              ...
              nfold = 5, # Use 5 fold cross-validation
               ...
               eval_metric = "mae") # Set evaluation metric to use
```
Best iteration was 7, will go to 150 to make sure we get everything.
...
Go with child weight 30 and max depth 10.
...
Gamma of 0.05 appears to be best.
...
Best was round 8, sticking with the same number of rounds.
...
Best is Subsample = 1 and Column Sapmle by Tree = 0.8
...
```{r}
boost_preds_tuned <- predict(bst_final, dtest) # Create predictions for xgboost model
